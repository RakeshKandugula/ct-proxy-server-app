...
function convert(
  ...
  else if (supplierName === "testsupplier1" || supplierName === "JOUTSEN_FINLAND_OY") {
   ...
   //  Decode range and start at Excel row 7 (0-based index 6)
    ...
    data = XLSX.utils.sheet_to_json(workbook.Sheets["Sheet1"], { header: 1 });
     ...
     data = XLSX.utils.sheet_to_json(sheet, { header: 1, range: fullRange });
      ...
      fullRange.s.r=findHeaderRow(sheet);
     data = XLSX.utils.sheet_to_json(sheet, { header: 1, range: fullRange });
    }else if(supplierName==="FB-TOOLS_OY"){
      const sheet = workbook.Sheets[sheetName];
      const fullRange = XLSX.utils.decode_range(sheet['!ref']);
      fullRange.s.r=findHeaderRow(sheet);
     data = XLSX.utils.sheet_to_json(sheet, { header: 1, range: fullRange });
    }else if(supplierName==="MONDIALPELLI_S.R.L."){
      const sheet = workbook.Sheets[sheetName];
      const fullRange = XLSX.utils.decode_range(sheet['!ref']);
      fullRange.s.r=findHeaderRow(sheet);
     data = XLSX.utils.sheet_to_json(sheet, { header: 1, range: fullRange });
     ...
     data = XLSX.utils.sheet_to_json(sheet, { header: 1, range: fullRange });
    }else if(supplierName==="RODEL_S.P.A."){
      const sheet = workbook.Sheets[sheetName];
      const fullRange = XLSX.utils.decode_range(sheet['!ref']);
      fullRange.s.r=findHeaderRow(sheet);
      //console.log(fullRange.s+"--"+supplierName);
     data = XLSX.utils.sheet_to_json(sheet, { header: 1, range: fullRange });
    }else if(supplierName==="ROHNISCH_SPORTSWEAR_AB"){
      const sheet = workbook.Sheets[sheetName];
      const fullRange = XLSX.utils.decode_range(sheet['!ref']);
      fullRange.s.r=findHeaderRow(sheet);
      //console.log(fullRange.s+"--"+supplierName);
     data = XLSX.utils.sheet_to_json(sheet, { header: 1, range: fullRange });
  }else if(supplierName==="BERG_BRAND_MANAGEMENT_ApS"||supplierName==="PYRENEX_SAS" ||supplierName==="ROHNISCH_SPORTSWEAR_AB"){
      const sheet = workbook.Sheets[sheetName];
      const fullRange = XLSX.utils.decode_range(sheet['!ref']);
      fullRange.s.r=findHeaderRow(sheet);
      console.log(fullRange.s+"--"+supplierName);
